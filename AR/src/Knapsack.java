...
public class Knapsack {
	...
	public static void main(String[] args) {
		...
		int[] weights = {40, 60, 50, 30};
		int capacity = 120;

		int[][] solution = getSolutionMatrix(weights, capacity);
	 ...
	 *
	 */
	private static int[][] getSolutionMatrix(int[] weights, int capacity) {
		int[][] matrix =  new int[weights.length + 1][capacity + 1];
		for(int i = 1; i <= weights.length; i++) {
			for (int j = 0; j <= capacity; j++) {
				if (j - weights[i-1]  >= 0) {
					matrix[i][j] = Math.max(matrix[i-1][j], weights[i-1] + matrix[i-1][j-weights[i-1]]);
				} else {
